...
* **The Dynamic Prompt Builder:** Instead of a static system prompt, our code dynamically constructs the prompt at runtime. It takes the $K$ retrieved emails from ChromaDB and stitches them into the LLM's system prompt as a formatted list of historical examples (e.g., `Incoming:` followed by `Reply:`). This ensures the LLM's "memory" is perfectly tailored to the specific context of the incoming email every single time.
