...
When a new email arrives, we query ChromaDB for the top 3 similar emails, and pass them to our **Dynamic Prompt Builder** before sending to **Groq** (`llama-3.3-70b-versatile`). 
* **The Dynamic Prompt Builder:** Instead of hardcoding a massive static prompt, the system acts as a Dynamic Prompt Builder. At runtime, it intercepts the incoming email, queries the Vector DB for the $K$ most relevant historical examples, and programmatically injects them into the LLM's system prompt as Few-Shot examples. This ensures the LLM's "context window" is only filled with highly relevant data perfectly tailored to the incoming email, drastically reducing hallucinations and mirroring the company's exact tone.
* **Prompt Engineering Technique:** We utilize **Dynamic Few-Shot Prompting**. The retrieved past emails are injected directly into the prompt as examples. We also strictly separate the `system` and `user` roles—the system prompt handles persona ("expert executive assistant"), formatting rules, and the dynamic examples, while the user prompt only contains the newly arrived email data. This prevents prompt injection and hallucinations.
...
3. **Embedding Model (Latency vs. Accuracy):** We used `all-MiniLM-L6-v2` (an 80MB model). *Why not the best-in-market?* For a free UI deployment running on a basic CPU, snappy UI latency is critical. State-of-the-art open-source models like `BAAI/bge-large-en-v1.5` are 1.3+ GB and cause massive latency spikes on CPU-only servers. **Next step:** In a funded production environment, we would upgrade to OpenAI's `text-embedding-3-large` (which offloads compute to their servers via API) or host `BGE-large` on dedicated GPU hardware, trading a slight increase in latency/cost for top-tier MTEB semantic retrieval accuracy.
...
5. **Dataset Quality & Corpus Expansion:** The underlying data is sourced from the [Enron Email Reply Dataset on Kaggle](https://www.kaggle.com/datasets/oanannv/enron-email-reply-dataset). Due to time constraints, we used a raw subset. **Next step:** With more time, we would heavily clean the corpus (removing automated replies, out-of-office messages, and malformed signatures) and dramatically expand the corpus size. Because RAG relies entirely on retrieved context, a cleaner, larger corpus guarantees significantly higher quality few-shot examples and, consequently, a much better final response.
